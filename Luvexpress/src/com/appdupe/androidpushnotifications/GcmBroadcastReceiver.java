			...
			if (action.equals("3")) {
				...
				if (bFlagForCurrent) {
					...
					homeIntent.putExtra("MESSAGE_FOR_PUSH_MESSAGETYPE",
							strMessageType);
					homeIntent.putExtra("MESSAGE_FOR_PUSH_MESSAGEID",
							strMessageID);
					homeIntent.putExtra("MESSAGE_FOR_PUSH_SENDERNAME",
							strSenderName);
					homeIntent.putExtra("MESSAGE_FOR_PUSH_DATETIME",
							strDateTime);
				...
				for (int i = 0; i < likesList.size(); i++) {
					...
					AppLog.Log(TAG,
							"GcmBroadcastReceiver doInBackground imageFile is profile "
									+ imageFile.isFile());
					com.appdupe.pair.utility.Utility.addBitmapToSdCardFromURL(
							likesList.get(i).getpPic().replaceAll(" ", "%20"),
							imageFile);
			...
			if (strMessageType.equals("2")) {
				...
				try {
					...
					String messageResponse = httpRequest.postData(
							sendMessageUrl, sendMessageReqList);
