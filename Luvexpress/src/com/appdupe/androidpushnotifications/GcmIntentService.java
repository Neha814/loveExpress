...
//		"775207507530"
		...
		try {
			...
			if (!extras.isEmpty()) { // has effect of unparcelling Bundle
				...
				if (GoogleCloudMessaging.MESSAGE_TYPE_SEND_ERROR
					...
					/*
					 * for (int i=0; i<5; i++) { Log.i(TAG, "Working... " +
					 * (i+1) + "/5 @ " + SystemClock.elapsedRealtime()); try {
	...
	private void sendNotification(String msg, Bundle extras) {
		...
		if (action.equals("3")
				&& preferences.getInt(Constant.KEY_SHOW_MATCH, 1) == 1) {
			notificationManager.notify(0, notification);
		}
		else if (preferences.getInt(Constant.KEY_SHOW_MESSAGE, 1) == 1) {
			notificationManager.notify(0, notification);
		}
	}
