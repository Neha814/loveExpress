...
import com.appdupe.pair.pojo.GridViewData;
import com.appdupe.pair.pojo.ListviewAlubumData;
	...
	public static PreferenceModel parsePreferenceResponse(String response) {
		...
		try {
			...
			if (object.getInt("errFlag") == 0) {
				model.setErrFlag(0);
				if (object.getString("prLAge").equalsIgnoreCase("null")
						&& object.getString("prLAge").equalsIgnoreCase(null)
						&& object.getString("prLAge").equalsIgnoreCase(""))
					model.setPrefLowerAge(0);
				else
					model.setPrefLowerAge(object.getInt("prLAge"));

				if (object.getString("prRad").equalsIgnoreCase("null")
						&& object.getString("prRad").equalsIgnoreCase(null)
						&& object.getString("prRad").equalsIgnoreCase(""))
					model.setPrefRadius(0);
				else
					model.setPrefRadius(object.getInt("prRad"));
					

				if (object.getString("prSex").equalsIgnoreCase("null")
						&& object.getString("prSex").equalsIgnoreCase(null)
						&& object.getString("prSex").equalsIgnoreCase(""))
					model.setPrefSex(0);
				else
					
					model.setPrefSex(object.getInt("prSex"));

				if (object.getString("prUAge").equalsIgnoreCase("null")
						&& object.getString("prUAge").equalsIgnoreCase(null)
						&& object.getString("prUAge").equalsIgnoreCase(""))
					model.setPrefUpperAge(0);
				else
					model.setPrefUpperAge(object.getInt("prUAge"));

				if (object.getString("sex").equalsIgnoreCase("null")
						&& object.getString("sex").equalsIgnoreCase(null)
						&& object.getString("sex").equalsIgnoreCase(""))
					model.setSex(0);
				else
					model.setSex(object.getInt("sex"));

				if (object.getString("search").equalsIgnoreCase("null")
						&& object.getString("search").equalsIgnoreCase(null)
						&& object.getString("search").equalsIgnoreCase(""))
					model.setSearchon(0);
				else
					model.setSearchon(object.getInt("search"));

				if (object.getString("prLat").equalsIgnoreCase("null")
						&& object.getString("prLat").equalsIgnoreCase(null)
						&& object.getString("prLat").equalsIgnoreCase(""))
					model.setPrLat("0");
				else
					model.setPrLat(object.getString("prLat"));

				if (object.getString("preLong").equalsIgnoreCase("null")
						&& object.getString("preLong").equalsIgnoreCase(null)
						&& object.getString("preLong").equalsIgnoreCase(""))
					model.setPreLong("0");
				else
					model.setPreLong(object.getString("preLong"));

				if (object.getString("loginLat").equalsIgnoreCase("null")
						&& object.getString("loginLat").equalsIgnoreCase(null)
						&& object.getString("loginLat").equalsIgnoreCase(""))
					model.setLoginLat("0");
				else
					model.setLoginLat(object.getString("loginLat"));

				if (object.getString("loginLong").equalsIgnoreCase("null")
						&& object.getString("loginLong").equalsIgnoreCase(null)
						&& object.getString("loginLong").equalsIgnoreCase(""))
					model.setLoginLong("0");
				else
					model.setLoginLong(object.getString("loginLong"));
				...
				ArrayList<Latlong_Data> data_latlong = new ArrayList<Latlong_Data>();
				if (object.has("data")) {
					JSONArray data = object.getJSONArray("data");
					if (data.length() > 0) {
						for (int i = 0; i < data.length(); i++) {
							JSONObject job = data.getJSONObject(i);
							Latlong_Data lr_lat = new Latlong_Data();
							lr_lat.setCity_name(job.getString("City_name"));
							lr_lat.setLat(job.getString("latitude"));
							lr_lat.setLong(job.getString("longitude"));
							lr_lat.setLocationId(job.getInt("location_id"));
							data_latlong.add(lr_lat);
						}
					}
				}
				model.setData(data_latlong);
